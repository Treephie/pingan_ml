...
# path_train = "data/train.csv"  # 训练文件
...
def process(clf):
    ...
    with open(path_test) as lines:
        with(open(os.path.join(path_test_out, "test.csv"), mode="wb")) as outer:
            ...
            for line in lines:
                ...
                # print pred
                writer.writerow([item[0], pred[0]]) # 随机值
...
if __name__ == "__main__":
    print("****************** start **********************")
    # 程序入口
    clf = model.init_model(path_train)
    process(clf)
